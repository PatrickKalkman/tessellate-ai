...
class Jigsaw:
    ...
    def generate_svg_jigsaw(self, outdirectory):
        ...
        # Check if inkscape is available, otherwise use svgpathtools only
        try:
            bboxes = subprocess.check_output(
                ["inkscape", "--query-all", "{}".format(fp.name)]).decode('utf-8')
            bboxes = bboxes.split("\n")[1:-1]
        except (subprocess.CalledProcessError, FileNotFoundError):
            # Fall back to manual bounding box calculation
            bboxes = self._calculate_bboxes_manually(fp.name)
        ...
        for p, path in enumerate(paths):
            if isinstance(bboxes[p], str):
                xmin, ymin, width, height = bboxes[p].split(",")[1:]
            else:
                xmin, ymin, width, height = bboxes[p]
            ...
            # Parse path commands more robustly
            path_d = path.d()
            ...
            # Default midpoints based on piece dimensions
            midpoint_top = metadata["PieceWidth"] / 2
            midpoint_right = metadata["PieceHeight"] / 2
            midpoint_bottom = metadata["PieceWidth"] / 2
            midpoint_left = metadata["PieceHeight"] / 2
    ...
    def _calculate_bboxes_manually(self, svg_path):
        """Calculate bounding boxes manually when inkscape is not available"""
